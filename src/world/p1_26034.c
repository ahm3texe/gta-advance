/* Release an entry that is invalid or whose key has changed — 0x08026034.
 * Phase 1; compared against the ROM twins. Measurement: data/phase1_evidence/0x08026034.json. */
#include "gba_types.h"
...
u32 FUN_08026034(Phase1EntryOwner *owner,u32 key)
{
    u8 *data = owner->data;
    Entry *entry;
    if (data[141] != 255) {
        entry = &gRam020246F0[data[141]];
        if (entry->active && entry->phase == key) return 1;
        ReleaseEntry(data[141]);
        data[141] = 255;
    }
    return 0;
}
