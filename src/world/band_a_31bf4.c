/* band_a.c'den AYRILDI. Sebep: agbcc_build.py bir cevirim birimini
 * min(tanimli fonksiyon adresi) noktasina linkliyor ve fonksiyonlari
 * KAYNAK SIRASINA gore diziyor. ROM'da bitisik OLMAYAN fonksiyonlar
 * ayni dosyada olursa ilki disindakiler yanlis adrese dusuyor ve `bl`
 * goreli ofsetleri kayiyor. Dis cagrisi olmayanlar yine eslesiyor,
 * cagri iceren bu ikisi eslesmiyordu. Ayri dosya = dogru link tabani.
 */
...
/* 0x08031BF4 -- 28 bayt
 * TEK BASINA BYTE-MATCHING (28/28, kendi ROM adresine linklenerek olculdu).
 * BU DOSYADA farkli: 3/28 bayt -- farkli olan uc bayt yalnizca tek `bl`
 * komutunun bagil ofset alani; dosya basligindaki yerlesim notuna bakin.
 *
 * Yigindaki tampona 79 baytlik bir dizgi kopyaliyor. FUN_0806DE10
 * (0x0806DE10) sokuldu: klasik `strncpy` -- once 4'er bayt kelime
 * kopyasi, sonra bayt bayt, kalani sifirla dolduruyor, `dest`
 * donduruyor. Donus degeri burada kullanilmiyor.
 *
 * TAMPON BOYUTU 84: ROM'un cercevesi `sub sp,#84`. Kopyalanan dizgi 80
 * bayt kapasiteli (79 bayt + sonlandirici), yani dort bayt fazlasi
 * cerceve icinde duruyor ve NE OLDUGU ROM'DAN OKUNAMIYOR. 81, 82, 83 ve
 * 84 boyutlarinin dordu de birebir ayni kodu uretiyor (olculdu); 80
 * `sub sp,#80` verip iki bayt sapiyor. Ayirt edici bilgi olmadigi icin
 * yuvarlak olan 84 secildi. */
