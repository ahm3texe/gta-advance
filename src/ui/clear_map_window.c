 ...
 * Verification: make c-match FILE=src/ui/clear_map_window.c
 */
...
/* 0x080311DC — BYTE-MATCHING. Fill 8 rows * 30 tiles with the empty tile.
 * Row stride is 32 entries/64 bytes, skipping two entries after each row.
 *
 * MEASURED: write DECREASING counters. The ROM uses movs r0,#30 / subs /
 * cmp #0 / bne, counting to zero. for (i=0; i<30; i++) stays increasing
 * in agbcc regardless of s32/u32 or for/do-while, yielding cmp r0,#29 / bls
 * (8/40 differing bytes). ClearHudFieldB's movs #5 / bge does not apply
 * (rule 49: do not copy a sibling's loop form). Increment the pointer in
 * source so its load precedes the constant (area_cleanup.c preheader ordering).
 */
#define MAP_WINDOW   ((vu16 *)0x06009B00)
#define MAP_ROWS     8
#define MAP_COLS     30
#define MAP_STRIDE   32

void ClearMapWindow(void)
{
    vu16 *p;
    u32 row;
    u32 col;
