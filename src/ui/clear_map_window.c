...
/* --------------------------------------------------------------------
 * 0x080311DC — BYTE-MATCHING.  8 satir x 30 karo bos karoyla dolduruluyor;
 * satir adimi 32 giris (64 bayt), yani her satirin sonunda iki giris
 * atlaniyor.
 *
 * OLCULEN: sayaclar KAYNAKTA AZALAN yazilmali.  ROM `movs r0,#30 /
 * subs / cmp #0 / bne` uretiyor, yani "n'den baslayip sifira" bicimi.
 * `for (i = 0; i < 30; i++)` -- s32 ya da u32, for/do-while farketmiyor --
 * agbcc'de ARTAN kaliyor ve `cmp r0,#29 / bls` cikiyor (8/40 bayt fark).
 * Kardes ClearHudFieldB'nin `movs #5 / bge` bicimi buraya UYMUYOR
 * (kural 49: kardesin dongu bicimini kopyalama).
 * Isaretci yuklemesi sabitten ONCE geldigi icin isaretci kaynakta
 * artiriliyor (area_cleanup.c'deki on-blok sirasi olcutu).
 * ------------------------------------------------------------------ */
